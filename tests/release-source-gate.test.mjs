...
function deploymentFixture({
  ...
  let snapshotFiles = [];
  let releasePreparerOptions = null;
  const sourceFiles = [{
    path: 'data/ontology-release-manifest.json',
  ...
  return {
    options: {
      ...
      releasePreparer: async (options) => {
        releasePreparerOptions = options;
        return ({
    ...
    releasePreparerOptions: () => releasePreparerOptions,
    manifest,
  };
}
...
test('preview receipt gates only production ontology promotion', async () => {
  ...
  assert.equal(ordinary.releasePreparerOptions().subjectOntologyV2Promotion, false);
...
test('page-evidence and ontology promotions execute as one prepared preview transaction', async () => {
  ...
  assert.equal(fixture.releasePreparerOptions().subjectOntologyV2Promotion, true);
});
