...
const corpusManifest = JSON.parse(await readFile(new URL('data/corpus-chunks/manifest.json', root), 'utf8'));
...
test('graph revision is bound to the exact sealed corpus manifest and release fingerprint', () => {
  for (const artifact of [core, academicIndex]) {
    assert.equal(artifact.input_fingerprints.corpus_manifest_sha256, corpusManifest.manifest_sha256);
    assert.equal(
      artifact.input_fingerprints.corpus_release_fingerprint_sha256,
      corpusManifest.release_fingerprint_sha256,
    );
  }
});
