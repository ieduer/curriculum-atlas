...
The proof also binds all 42 touched catalog identities across the complete governed work identity (ID, country, language, title, subject, stage, document type, version, issuer, issue/publication dates, and current status) and a separate complete canonical-artifact identity. Every governed identity key is explicit; absent keys no longer collapse to explicit null. A fixed Git-object baseline at `373627780303ba91ac7159f99b166c85f9a1b9af` independently pins those identities, the complete 149-document canonical-PDF universe, and all fields plus ordering of the 86-row OCR queue, so coordinated catalog/proof relabeling or artifact swaps cannot authorize themselves by recomputing hashes. A deliberate baseline change requires a separate reviewed promotion. The local gate physically runs `pdfinfo` against all 149 canonical catalog PDFs and exact-binds all 86 OCR queue entries to catalog path/hash/page metadata. Exactly the two named zero-prefix corrupt recoveries, and no caller-selected substitutes, map one-to-one to their named quarantine artifacts.

`data/source-recovery-online-receipt.json` is an exact-proof-bytes-bound 72-hour receipt for 6 official publication pages and 23 exact online artifacts. It records page and artifact redirect chains, status, MIME, bytes, SHA-256 and page href identity; every redirect chain must start at the request URL, remain contiguous and end at the declared final URL. The ICTR WAF exception is limited to the exact `fangan.html` and `pt.html` URLs at exact status 412. Placeholder hosts, 404s, other ICTR URLs/statuses, missing hrefs, non-contiguous redirects and stale receipts fail closed. Ordinary tests use injectable fetch fixtures and make no network request. `prepare-release.mjs` and `deploy-worker.mjs` independently require a fresh immutable receipt before any injected release preparer or Wrangler call. The proof, receipt, and schemas are versioned public release metadata, while source binaries remain in the private cache. `npm run assets:audit` validates both metadata layers automatically; release manifest construction refuses to omit them.
