...
- 左側學科下方的年份滑杆是唯一的時間控制器，用於裁剪同一張星圖。
- 學科／課程範圍決定星群軌道、縱深與顏色。
- 所有證據狀態共用完全相同的星體亮度、輪廓、光暈、動態和選中效果；證據狀態只進檢查器文案與資料欄。
- 文檔與頁碼是星點的證據，不是第二條軸線。
- 關係線平時不鋪滿星圖；點選概念後只顯示該概念族的實線演進鏈，長跨度線直接標註起訖年份與關係類型。
...
| 點選強光 | 被選概念及其同一 `language-practice-domain` 族譜全部 episode | 只表示受控比較集合，不表示語義等同 |
...
- 左側固定順序：12 個受控學科分面 → 年代按鈕與縱向年份滑杆 → 星圖檢索 → 百年演進／概念關係／概念深挖 → 版本・資料／研究・討論 → 百年資料與證據狀態。
- 右側：不設永久工具軌，完整留給星圖與點選後的證據檢查器。
- 中央：唯一星圖 Canvas。
- 所有 episode 共用同一套光核、光暈、呼吸、尖芒、標籤密度、懸停、點選、鏡頭與年份裁切；不得為候選增加虛線環或第二套材質。
- 點擊星體：打開觀測檢查器，顯示文件、版本、頁碼、詞面、證據狀態和候選邊界；若該概念屬於百年族譜，同時點亮族內全部年份、概念與關係。
...
19 個概念族合計 78 個受控概念、1,034 個 1902–2022 episode memberships。12 科名稱族都必須同時有 2001 年前 OCR 節點和 2001 年後教育部編目節點；編目標題只負責接上當代名稱，不代替正文證據。所有數字由 builder 生成，不手填進前端。
...
4. **抽取詞面**：使用版本化 `concept-lexicon.json`、`concept-evolution-families.json#historical_concepts` 與 `#course_identity_concepts` 的受控 surface form；最長詞面優先，避免長詞被重複計作短詞；保留 physical page、surface、count、item identity 與唯一 display facet。
...
| `data/concept-evolution-families.json` | 兩個固定概念層級、19 族、歷史詞面與非因果轉寫配置 | 人工受控配置 |
...
| P0 | 12 科由課程名稱層下鑽到各科實踐／內容／能力領域 | 每科都有固定粒度、版本化詞表、bounded-item OCR 與誤命中 review；不得跨粒度硬連 |
...
2. 為數學、外語、思政、歷史、地理、科學、技術、勞動、藝術、體育等專科匯編逐冊解析 bounded item，從已上線的「課程名稱」層下鑽到與語文相同粒度的實踐／內容／能力概念族。
