...
- 左側默認折疊抽屜：12 個受控學科分面 → 星圖檢索及同源鍵盤列表 → 百年演進／概念關係／概念深挖 → 版本・資料／研究・討論 → 百年資料與證據狀態。
...
2026-07-23 受控快照為：名義 86 份／11,847 頁；物理去重 85 份／11,779 頁；完整整卷 83 份；runtime 已完成 10,690 頁；候選覆蓋 10,770 頁；候選缺口 1,077 頁；雙見證審計 6,947 頁；人工維持不可引文 4 頁；引文就緒 0 頁。缺口只能以三個顯式區間存在：地理 97–518、數學 337–697、思想政治 129–422。這些數字更新時必須同時修改標準、帳本與驗證收據，禁止只改前端文案。
...
4. candidate Schema、episode diff、24 項數據質檢、靜態性能、tests、typecheck、asset audit、release manifest、secret scan、Wrangler dry-run；
5. commit / push；
...
| P0 | 完成三個顯式 OCR 候選缺口 | 地理 97–518、數學 337–697、思想政治 129–422 逐頁回填；帳本仍維持 zero silent missing |
| P0 | 高價值候選的頁／版次人工裁決與引文晉級 | candidate、citation、semantic、first/disappearance/influence 五個 gate 可逐條追溯，不以 OCR complete 自動升級 |
| P1 | archive item identity 人工抽查與版本核對 | 134 項有 review state，不改 stable ID |
| P1 | 完成星圖 runtime 性能基線 | preview 桌面／手機 ready、transfer、draw p95、long task 全部低於不可覆蓋預算 |
| P2 | 24 小時後驗自動化 | aggregate health/Pulse only，無敏感資料 |

已完成且已進發布門的原技術債：正式 candidate JSON Schema、同源鍵盤星點列表、靜態 Canvas 預算、相對凍結正式版的 episode diff receipt。

## 18. 後續路線

1. 完成剩餘 OCR 文件，保持同一星圖的候選星持續增長。
2. 對已解析的 12 科 341 個 bounded items 做分層人工抽查、誤命中 review、物理頁／版次裁決；新增專科來源仍沿用相同 builder 與固定粒度族。
