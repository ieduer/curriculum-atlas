...
- 左側默認折疊抽屜：11 個公開檢索分面 → 星圖檢索及同源鍵盤列表 → 百年演進／概念關係／概念深挖 → 11 條學科設置百年鏈及 9 個來源明示里程碑 → 版本・資料／研究・討論 → 百年資料與證據狀態。資料庫仍保留 12 個課程形態身份，其中「歷史」與「歷史與社會」只在公開檢索層合併。
...
`public/data/discipline-lifecycle.json` v2 先按學科層級組織 11 條 1902–2022 課程名稱鏈，再掛接科學／分科、藝術／音樂美術、勞動獨立、信息科技獨立和道德課程一體化等 9 個來源明示里程碑。事件必須有來源、事件類型、涉及課程形態與 claim boundary；點事件顯示全部參與學科的百年鏈，不得退化為單一年份。語文鏈的 1902「作文」只作課程表並列形態候選，不能把實踐層作文詞面批量提升為課程身份。
...
當前資料發布閘門為 39／39，百年模型專項閘門為 24／24；11／11 公開學科分面均具備課程名稱、實踐、內容、能力四層來源綁定模型。新增資料閘門要求 83 份 runtime-complete 文件／10,210 頁全部有終局 observation-builder 處置、12 個底層分面齊全、年份未決 0、462 個 bounded identity receipt 失敗 0、31→30 去重關係可重算；學科設置另要求 11 條公開學科鏈全部解析至 2022、9 個事件來源齊全、社会科合科編組只涉及思政／歷史／地理。互動閘門要求年代導航與年份對比互斥、實際資料年份可任意多選、1902／2022 首尾快捷可執行、所有有狀態篩選第二次點擊可恢復、不得退回 range-only 拖拽，且檢查器與時間塢必須把自身矩形交給 Canvas safe viewport 重新擬合。暗／亮主題都要保持文字及選中實線 AA 對比和同一星點語義。

v20/v47 同一發布候選的真實 preview runtime 收據為：桌面 1440×1000，ready 546.4 ms、draw p95 5.2 ms、long task 0／0 ms；手機 390×844，ready 493.5 ms、draw p95 3.4 ms、long task 0／0 ms。兩端均為單一 Canvas、10 個星圖內年代顯隱階段、左側默認折疊、零橫向溢出。正式域另逐一路由核對 `/compare`、`/sources`、`/archive` 與代表性 1902 `/historical/` 條目：前兩者無 `page_count` 例外，目錄精確 461 個入口，條目顯示來源頁段、統一用戶登入、原圖優先與候選 OCR 不可引文邊界；桌面及手機 console errors/warnings 均為 0。
...
| P1 | archive item identity 全量機器一致性核對 | 462 項均綁定 source hash、物理頁、標題邊界與 stable ID；134 個 seed 對應有可重現 receipt，衝突 fail closed |
