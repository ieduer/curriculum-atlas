...
- 點擊「概念深挖」中的任一層級星點：底部同一年代控制器顯示該概念的來源綁定年份；點年份即裁切同一 Canvas 並重新擬合，不生成第二條時間軸。
...
6,947 頁不是「尚未做第二次 OCR」，而是雙見證已存在但舊 producer 對每頁都輸出空 `critical_fields`，因此 gate 正確地全部 fail closed。v15 將其完整分為：66 頁機器一致抽樣池、5,028 頁文字／結構衝突裁決、1,780 頁表格單元重建、73 頁空白視覺確認，未分類 0。四類均仍不可引文；只有對應頁的版本、圖像和人工裁決完成後才可逐頁晉級。

當前資料發布閘門為 28／28，百年模型專項閘門為 18／18；11／11 公開學科分面均具備課程名稱、實踐、內容、能力四層來源綁定模型。檢查通過只表示模型與候選觀測可發布，不表示 6,947 頁已完成人工引文審查。
...
| `data/century-model-validation.json` | 11 公開分面、四層深挖、歷史沿革、OCR 分流、檢查器與年份關聯的 v15 專項收據 | `century-model:validate` / `century-model:check` |
...
| `public/historical-stages.js` | 1902–2022 單一導航分期；Canvas、底部顯隱、archive 分組與無障礙年份文案共用 | `tests/historical-stages.test.mjs` |
