...
- 年份多選控制器橫向嵌入星圖底部，列出實際有資料的年份；空選代表全部顯示，可任意選擇兩個或多個年份精確比較，亦可一鍵選擇首尾年份。它只裁剪同一張星圖，不得再放回左側形成「百年縱軸」。
- 學科／課程範圍決定星群軌道、縱深與顏色。
...
- 星圖底部：橫向階段按鈕與實際資料年份多選；它控制同一 Canvas 的精確年份集合，不是第二條資料軸。
...
- OCR 同時報告名義與物理去重分母、完整頁、候選覆蓋頁、明確缺口、人工裁決頁與引文頁；
...
6,947 頁不是「尚未做第二次 OCR」，而是雙見證已存在但舊 producer 對每頁都輸出空 `critical_fields`，因此 gate 正確地全部 fail closed。v15 將其完整分為：66 頁機器一致抽樣池、5,028 頁文字／結構衝突裁決、1,780 頁表格單元重建、73 頁空白視覺確認，未分類 0。人工不是重新抄寫 6,947 頁：66 頁一致池可在版次、源 PDF、物理頁與頁圖雜湊固定後，用簽名的有界抽樣決定是否整批晉級；其餘頁面則因 6,527 頁數字序列衝突、5,283 頁題名衝突、5,436 頁正規化一致度未達門檻、1,780 頁表格格位不確定或 73 頁需確認真空白而進入相應裁決。OCR 字符相似不等於可逐字引用：版次／物理頁身份、專名與日期數字、表格行列結構、頁面是否真空白都必須由頁圖證據裁決。四類在裁決完成前仍不可引文，但候選星、詞面檢索和頁面定位可以完整公開，不得把候選資料冒充正式引文。

當前資料發布閘門為 28／28，百年模型專項閘門為 20／20；11／11 公開學科分面均具備課程名稱、實踐、內容、能力四層來源綁定模型。新增互動閘門要求底部年份為實際資料年份的任意多選、1902／2022 首尾快捷可執行、不得退回 range-only 拖拽，且檢查器必須把自身矩形交給 Canvas safe viewport 重新擬合。檢查通過只表示模型與候選觀測可發布，不表示 6,947 頁已完成人工引文審查。
...
| `public/data/ocr-coverage-summary.json` | 可公開的候選覆蓋／審查分流摘要；前端資料工作台與左側狀態共用 | `ocr:review:triage` |
...
| P0 | 依四類隊列完成高價值候選的頁／版次人工裁決與引文晉級 | 66 頁抽樣池先建立有簽名的抽樣包；5,028 頁衝突、1,780 頁表格、73 頁空白逐頁裁決；candidate、citation、semantic、first/disappearance/influence 五個 gate 可逐條追溯 |
| P1 | archive item identity 人工抽查與版本核對 | 134 項有 review state，不改 stable ID |
| P2 | 24 小時後驗自動化 | aggregate health/Pulse only，無敏感資料 |

已完成且已進發布門的原技術債：正式 candidate JSON Schema、同源鍵盤星點列表、三個 OCR 候選缺口、6,947 頁隊列根因與完整分流、11 公開學科的四層深挖模型、學科分合事件層、靜態 Canvas 預算、真實 preview 桌面／手機 runtime 性能基線、相對凍結正式版的 episode diff receipt。

## 18. 後續路線

1. 把已完成候選 OCR 持續投影到同一星圖，任何新增頁只可增加來源綁定的同粒度候選觀測。
2. 對已解析的 12 個底層課程形態 bounded items 做分層人工抽查、誤命中 review、物理頁／版次裁決；公開查詢維持 11 分面，新增專科來源仍沿用相同 builder 與固定粒度族。
