...
當前固定輸入重建得到 25 個歷史觀測概念、433 個 1902–2000 observation episodes、1,754 條物理頁 evidence。跨 2001 邊界的 7 個 `language-practice-domain` 概念族包含 24 個受控概念、427 個 1902–2022 episode memberships；這些數字由 builder 生成，不手填進前端。
...
4. **抽取詞面**：使用版本化 `concept-lexicon.json` 與 `concept-evolution-families.json#historical_concepts` 的受控 surface form；最長詞面優先，避免「識字與寫字」同時重複計作短詞；保留 physical page、surface、count 和 item identity。
...
| `data/concept-evolution-families.json` | 同層概念族譜、歷史詞面與非因果轉寫配置 | 人工受控配置 |
