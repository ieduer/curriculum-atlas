...
Production 使用同一 desired-release artifact 重复链路：prechange receipts → `0008`/`0009`/secret 检查 → fenced exact corpus import → exact-tree Worker deploy → `.wrangler` environment evidence → coordinator D1/R2 activation → API/browser/dependency QA。Preview 和 production 中途不得换 Git HEAD 或 artifact bytes。
