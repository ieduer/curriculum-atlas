...
本地 `CURRICULUM_RELEASE_COORDINATOR_TOKEN` 与 Worker secret `RELEASE_COORDINATOR_TOKEN` 必须引用同一批准的秘密值，但不得打印、提交或写入报告。确认 `0008_release_ownership_fences.sql` 已应用且 pending 为 0；该 migration 分离 corpus/import publication fence，并把 R2 coordination schema 提升为 3。普通 owner expiry takeover 会增加 fence，旧 owner 随即失效；一旦 coordinator 取得最长 600 秒的 activation claim，所有受支持的 owner acquisition/release 都必须等待该 claim 释放或有界过期，不能在 inventory validation 与 conditional pointer put 之间插入接管。
