...
- DMITPro2 的 B-r1 1,259/3,182 是不可变 predecessor；其受审后继 B3 已完成并 settled 6 本汇编、3,182/3,182 页，等待与最终 A2 组成完整 14 卷 union 后通过 whole-document receiver。A2 已耐久生成 1,568/3,182 页（4/8 卷 complete、1 interrupted、3 retry_wait、0 failed page），又因 operator monitor assertion 主动冻结；当前 output inode `45748776` 保留原 attempt 6 与原 authority/grant，worker/monitor/llama 均不得擅自重启。后续只能使用经独立审查的 same-attempt forward continuation，不得另发 grant、重置 attempt 或覆盖既有页/state。
