...
- 举报处置用一次 D1 batch 完成唯一 claim、讨论状态、审计记录和最终状态；并发管理员不能重复处置同一开放举报。
...
3. 逐个 inventory kind 在 `0001`—`0009` 的真实迁移 schema 上执行，不能只用接受任意 SQL 的 mock。
4. 举报处置后 `comment_reports`、`comments` 与 `content_audit_log` 三者一致；重复处置不产生第二条审计。
5. API 响应、Worker 日志和浏览器控制台均不得出现 cookie、session、IP、原始用户/查询哈希或秘密。
6. preview 用真实 admin/non-admin 账号逐页验收，再以同一工件进入 production；管理页面不是公开导航入口，`robots.txt` 保持禁止 `/admin`。
