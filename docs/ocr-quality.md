...
PDF containers are not presumed independent merely because their file hashes differ. Exact-edition or independent evidence always requires a non-null artifact receipt that points to the retained online PDF and binds the controlled source identity and artifact ID. The validator safely reads both actual PDF byte streams, renders every page itself with the fixed `pdftoppm-png-144dpi-cropbox-v1` profile, hashes every rendered page, and recomputes the ordered aggregate and count. Receipt-only `a/b/c` hashes are not evidence. Equal recomputed page-image sequences force `same_artifact_mirror` and `independent_for_decision=false`; a metadata-only or container-only repackage can never promote citation. A human scan review must resolve every engine conflict and high-risk field. HTML tables, Markdown pipe tables and flattened two-or-more-column tables all force an explicit page type plus a complete row-major cell manifest whose dimensions, blank cells, text hashes, extracted accepted-text cells and manifest hash are checked. `manual_grid` is not an extraction format and cannot bypass parity. Stable-fact or different-edition evidence is retained as a scoped decision but cannot promote page wording. A warning decision retains its uncertainty note and remains unavailable to citation-locked AI.
