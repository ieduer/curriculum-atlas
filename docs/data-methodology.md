...
目录的原始 `subject` 是来源标签，不是受控筛选值。构建器先应用 `data/concept-model-v2.json` 的来源标签映射与文档级 override，再把合格的 `subject` / `assessment_subject` 写成精确 `canonical` 身份。星图另外读取 `subject_facet_groups` 形成 12 个展示组；`汉语→语文`、各外语语种→外语、历代思想政治/道德法治名称→思想政治与道德法治、科学分科→科学类、信息/通用技术→技术只影响显隐与配色，不改变 canonical、stable ID、官方代码、版本比较或证据检索。特色课程、历史“技术”课程与综合实践活动采用 `entity_kind=curriculum_course`、`facet_eligible=false`，由 `course_families` 和 `course_to_subject_links` 保存课程族及关联学科，并在 episode 中显式写入 `course_entity`。课程方案、考试评价、考试大纲、跨学科政策、汇编卷和来源目录则继续作为非课程 scope；两类都不会污染学科 facet。
