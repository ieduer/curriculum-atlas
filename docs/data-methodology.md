...
精确页放行不依赖人工裁决。机器门禁必须同时绑定原始 PDF、页图、主 OCR、独立 OCR、文件身份和各层 SHA-256；两路 OCR 的规范化全文必须逐字完全一致，发布构建器还会重新读取页图和文本并复算所有哈希。通过后只在 `data/page-publication-manifest.json` 显式打开该页；未列出的页一律默认封闭。官方或学术在线证据只用于确认文件身份与版次关系，不得覆盖扫描图像上的历史措辞。

任何文本或表格冲突都会得到终态 `fail_closed_omission`，不进入人工待审队列，也不生成正式引文。旧数据中的 `human_judgment_with_warning` 仅保留为历史审计记录，始终 `citation_allowed=0`。当前机器裁决分母为 6,947 页：31 份精确收据映射为 30 个唯一可引页，其余页全部自动封闭。
...
- OCR 通过抽样不等于整份文档通过；必须逐页放行，文档级 `citation_allowed=1` 也不能绕过页级门禁。
- 在线文本可能来自后续修订版，必须显式标注版次关系。
- 当前历史扫描只开放 30 个机器双证据完全一致页；其余正文仍以候选观察或终态封闭状态存在。
- 教育部修订动态不等于新标准已经发布。
