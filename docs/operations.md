...
标准流程见 [`deployment.md`](deployment.md)：最终 commit/push → clean exact-HEAD artifact → prechange Time Travel/pointer receipts → `0008`/secret → fenced exact corpus import → exact-tree Worker deploy → `.wrangler` evidence → coordinator conditional R2 activation → API/browser/dependency QA。
...
该最后验证事件是 legacy v10 状态；`0008`、desired-release v2、coordinator conditional activation 与 owner/fence takeover 尚未部署时必须继续标为未上线，不能用本地测试代替 live verification。
