...
- Production Worker version `43bf06ad-4eff-4b4b-9fbf-142e3fddbc2a`，deployment `30b00a0c-b35d-453d-b7df-4008778e3653`，Assets Git `38c9c4e9ffdcc24c13efb38b2399e572c5f7ab4a`。
- Preview Worker version `091d8e71-a83b-4236-a654-7ae57db2df28`，deployment `e7cdd602-7eaa-4210-839a-dadd7bc26c35`，Assets Git `9afa6565975b17379a2f6f6f371d0577988b4f5f`。
- Production R2 current 为 `release-9cb02f77c06ee0535e7981a22b312373`；preview 为 `release-841a528f0086ce69f2f7a6f2d07c0999`。
- 环境证据提交为 `d497b713e4d124f9fe6d1c6ee9a67f5eddc02292`；完整本地发布链通过 554/554 Node tests、14/14 Python tests、TypeScript、asset audit、release manifest 与 Wrangler dry-run。
...
- 单一星图已合并百年候选投影：134 个 archive items 产生 48 个 candidate episodes、66 条物理页 evidence、38 条同词面观察序列与 5 条同篇共现线；全部 citation/semantic/influence 闸门关闭。公开 OCR 状态同时显示 10 册 / 4,078 页已完成，但不代表全队列完成或引文开放。
...
- 首页只有一个 Canvas；时间只作为 episode 年份坐标和右侧筛选。134 个历史篇目只能进入 `/archive` / evidence，`/timeline` 必须改写到 `/archive`，DOM 不得出现 `#century-timeline` 或 `#century-track`。
- `century-observation-layer.json#star_projection` 必须保持 48/66/38/5 当前计数、每星至少一条 evidence、候选与边全部 fail closed；OCR 新批次只经确定性 builder 追加到同一星图。
...
Release evidence 观测于 2026-07-23 UTC，Git evidence commit `d497b713e4d124f9fe6d1c6ee9a67f5eddc02292`；两端 health 200、migrations 0001–0007、corpus ready，production/preview Worker、deployment 与 Assets Git 见本文检查点。D1、R2 pointer/objects、VPS、OCR runtime 与共享 hub 本次均无 mutation。

Production 真实浏览器证明：1280 viewport 主星图无横向溢出、无第二 timeline DOM、`字课 · 1902` 候选星可回到《钦定蒙学堂章程》PDF physical p.17，console errors 为 0；390×844 证明 `scrollWidth=innerWidth=390`、主星图与 `/archive` 工作台可用、archive rows=134、console errors/warnings 为 0。Preview 同样通过 `/archive`、`/historical/<id>` 与旧 `/timeline` 改写。两个任务浏览器均已关闭；orphan dry-run 只发现另一任务明确持有的 `xue-live-r6`，未执行清理。
