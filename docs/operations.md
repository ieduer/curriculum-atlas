...
- 单一星图已合并百年候选投影：134 个 archive items 产生 433 个历史概念观察、1,754 条物理页 evidence、408 条同词面观察序列与 1,763 条同篇共现候选；7 条同层概念演进族覆盖 427 个观察点，含 237 条同词面实线与 17 条编辑对应实线。全部 citation/semantic/influence 闸门关闭。公开 OCR 状态同时显示 10 册 / 4,078 页已完成，但不代表全队列完成或引文开放。
...
- `century-observation-layer.json#star_projection` 必须保持 433/1,754/408/1,763 当前计数、每星至少一条 evidence、候选与边全部 fail closed；`concept-evolution-families.json` 必须保持 7 族 / 427 memberships，所有对应关系保持 nonsemantic/noncausal。OCR 新批次只经确定性 builder 追加到同一星图。
