...
- `release/current.json`、release manifest、当前 v2 policy 的 16 个 versioned objects 与唯一 desired-release artifact 必须逐字节匹配；schema-2 release 还必须核对 pointer fence/ETag/version 和每个对象的 coordinator metadata。下列 17-object 数字仅描述 2026-07-17 legacy checkpoint。
...
标准流程见 [`deployment.md`](deployment.md)：最终 commit/push → clean exact-HEAD artifact → prechange Time Travel/pointer receipts → bridge Worker 与 dual-schema receipt → 按序执行 `0008`/`0009` → fenced corpus 与 content-addressed graph no-pointer staging → coordinator 对 D1/R2 条件激活 → environment evidence/postflight → API/browser/dependency QA。
...
- 绕过 D1 owner/fence 或 authenticated Worker coordinator 直接写入/删除 R2 pointer 与 immutable objects；
...
该最后验证事件是 legacy v10 状态；`0008`/`0009`、desired-release v2、coordinator conditional activation 與 owner/fence takeover 尚未部署时必须继续标为未上线，不能用本地测试代替 live verification。
...
A2 timeout-recovery continuation 的唯一发布、监控、归档与回滚步骤见 [`remote-ocr-a2-deployment.md`](remote-ocr-a2-deployment.md)。该手册把 authority/grant、successor inode、seed dry-run/apply、alert rebinding 与 freeze 边界写成顺序硬门，不能只复制 systemd unit 后直接启动。
