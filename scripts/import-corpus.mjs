...
export async function validateCorpusManifestSourceBindings(manifestInput, { root } = {}) {
  ...
  const queue = JSON.parse(await readFile(source('data/ocr-queue.json'), 'utf8'));
  const compendiumItemBoundaries = JSON.parse(
    await readFile(source('data/compendium-item-boundaries.json'), 'utf8'),
  );
  const compendiumValidation = validateCompendiumItemBoundaries(compendiumItemBoundaries, {
    catalog,
    queue,
    onlineVerifications: { samples: onlineVerificationSamples },
  });
  if (!compendiumValidation.valid) {
    throw new Error(`corpus compendium boundary source failed validation: ${JSON.stringify(compendiumValidation.errors)}`);
  }
  const compendiumProjection = buildCompendiumCorpusProjection(
  ...
  for (const record of catalog.documents) {
    ...
    for (let pageIndex = 0; pageIndex < pages.length; pageIndex += 1) {
      ...
      const pageDisplayAllowed = nativeText || acceptedOcr.pages[pageIndex].display_allowed;
      const embeddedItem = compendiumItemForPage(
        compendiumProjection,
        record.id,
        pageIndex + 1,
      );
      const isCompendiumCarrier = compendiumProjection.byDocument.has(record.id);
      const displayAllowed = pageDisplayAllowed && (!isCompendiumCarrier || Boolean(embeddedItem));
...
export function buildCorpusImportStartSql(manifestInput, {
  ...
  const releaseIdSql = sql(manifest.release_id);
  const exactReady = `(EXISTS(
  SELECT 1 FROM corpus_import_releases
  WHERE release_id=${releaseIdSql}
    ...
    AND expected_documents=${manifest.documents}
    AND expected_paragraphs=${manifest.paragraphs}
    AND expected_fts_rows=${manifest.fts_rows}
    AND expected_page_gates=${manifest.page_publication_gates}
    AND expected_displayed_paragraphs=${manifest.displayed_paragraphs}
    AND accepted_ocr_documents=${manifest.accepted_ocr_documents}
    AND expected_chunks=${manifest.sql_chunks}
    AND expected_core_counts_json=${sql(expectedCoreCountsJson)}
    AND actual_documents=expected_documents
    AND actual_paragraphs=expected_paragraphs
    AND actual_fts_rows=expected_fts_rows
    AND actual_page_gates=expected_page_gates
    AND actual_displayed_paragraphs=expected_displayed_paragraphs
    AND actual_chunks=expected_chunks
    AND actual_core_counts_json=expected_core_counts_json
    AND (SELECT value FROM site_meta WHERE key='current_corpus_release_id')=${sql(manifest.release_id)}
    AND (SELECT value FROM site_meta WHERE key='current_corpus_manifest_sha256')=${sql(manifest.manifest_sha256)}
    AND (SELECT value FROM site_meta WHERE key='corpus_import_state')='ready'
)
AND (SELECT COUNT(*) FROM documents WHERE corpus_release_id=${releaseIdSql})=${manifest.documents}
AND (SELECT COUNT(*) FROM paragraphs WHERE corpus_release_id=${releaseIdSql})=${manifest.paragraphs}
AND (SELECT COUNT(*) FROM paragraph_fts)=${manifest.fts_rows}
AND (SELECT COUNT(*) FROM page_publication_gates WHERE corpus_release_id=${releaseIdSql})=${manifest.page_publication_gates}
AND (SELECT COUNT(*) FROM paragraphs WHERE corpus_release_id=${releaseIdSql} AND display_allowed=1)=${manifest.displayed_paragraphs}
AND (SELECT COUNT(DISTINCT document_id) FROM page_publication_gates
  WHERE corpus_release_id=${releaseIdSql} AND publication_basis='accepted_ocr_page_manifest')=${manifest.accepted_ocr_documents}
AND (SELECT COUNT(*) FROM corpus_import_chunks WHERE release_id=${releaseIdSql})=${manifest.sql_chunks}
AND ${coreCountsJsonSql(releaseIdSql)}=${sql(expectedCoreCountsJson)})`;
...
export function corpusImportReleaseReadySql(manifestInput) {
  ...
  WHERE release_id=${releaseId}
    AND release_fingerprint_sha256=${sql(manifest.release_fingerprint_sha256)}
    AND manifest_sha256=${sql(manifest.manifest_sha256)} AND state='ready'
    AND actual_documents=${manifest.documents}
    AND actual_paragraphs=${manifest.paragraphs}
    AND actual_fts_rows=${manifest.fts_rows}
    AND actual_page_gates=${manifest.page_publication_gates}
    AND actual_displayed_paragraphs=${manifest.displayed_paragraphs}
    AND actual_chunks=${manifest.sql_chunks}
    AND actual_core_counts_json=${sql(JSON.stringify(manifest.core_table_counts))}
)
  AND (SELECT value FROM site_meta WHERE key='current_corpus_release_id')=${releaseId}
  AND (SELECT value FROM site_meta WHERE key='current_corpus_manifest_sha256')=${sql(manifest.manifest_sha256)}
  AND (SELECT value FROM site_meta WHERE key='corpus_import_state')='ready'
  AND (SELECT COUNT(*) FROM documents WHERE corpus_release_id=${releaseId})=${manifest.documents}
  AND (SELECT COUNT(*) FROM paragraphs WHERE corpus_release_id=${releaseId})=${manifest.paragraphs}
  AND (SELECT COUNT(*) FROM paragraph_fts)=${manifest.fts_rows}
  AND (SELECT COUNT(*) FROM page_publication_gates WHERE corpus_release_id=${releaseId})=${manifest.page_publication_gates}
  AND (SELECT COUNT(*) FROM paragraphs WHERE corpus_release_id=${releaseId} AND display_allowed=1)=${manifest.displayed_paragraphs}
