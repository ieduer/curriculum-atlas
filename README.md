...
- 发布前执行 34 项不可人工覆盖的数据细度／准确度核查、20 项百年模型／多年份／无遮挡检查、候选 JSON Schema、episode stable-ID diff 与星图性能预算；亮色四类选中实线对纸面背景均须达到 4.5:1。生产部署还必须持有与当前公开资产指纹一致的 preview 桌面／手机运行收据。任一失败都在 Wrangler 之前阻断新版。
