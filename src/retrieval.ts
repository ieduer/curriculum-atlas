...
export async function retrieve(env: Env, filters: SearchFilters): Promise<Passage[]> {
  ...
  try {
    const capability = await env.DB.prepare(`SELECT COUNT(*) AS count FROM sqlite_master
      WHERE type='table' AND name='embedded_items'`).first<{ count: number }>();
    embeddedItems = Number(capability?.count || 0) === 1;
  } catch {
    // Legacy test doubles do not expose sqlite_master; production D1 does.
  }
  const ftsSql = embeddedItems ? `SELECT p.id, COALESCE(ei.id, p.document_id) AS document_id,
       ...
       ORDER BY score ASC
       LIMIT ?` : `SELECT p.id,p.document_id,p.document_id AS parent_document_id,NULL AS embedded_item_id,
              d.title,dc.entity_kind,dc.taxonomy_entity_kind,dc.display_facet,dc.canonical_subject AS subject,
              COALESCE(dc.canonical_subject,dc.scope_label,dc.source_subject_label) AS entity_label,
              dc.subject_family,dc.scope_kind,dc.scope_label,d.version_label,
              p.page_number,p.source_locator,p.body,d.source_url,bm25(paragraph_fts) AS score
       FROM paragraph_fts
       JOIN paragraphs p ON p.id=paragraph_fts.paragraph_id
       JOIN documents d ON d.id=p.document_id
       JOIN page_publication_gates g ON g.document_id=p.document_id
        AND g.page_number=p.page_number AND g.corpus_release_id=p.corpus_release_id
       JOIN document_classifications dc ON dc.document_id=d.id
       WHERE paragraph_fts MATCH ?
         AND p.corpus_release_id=(SELECT value FROM site_meta WHERE key='current_corpus_release_id')
         AND d.corpus_release_id=p.corpus_release_id AND p.citation_allowed=1
         AND g.citation_allowed=1 AND d.citation_allowed=1
         AND (?='' OR (dc.taxonomy_entity_kind='subject' AND dc.canonical_subject=?))
         AND (?='' OR d.stage=?) ORDER BY score ASC LIMIT ?`;
  const match = ftsQuery(query);
  if (match) {
    const result = await env.DB.prepare(ftsSql).bind(match, subject, subject, stage, stage, limit).all<Passage>();
    if (result.results.length) return result.results;
  }

  const like = `%${query.replaceAll('%', '\\%').replaceAll('_', '\\_')}%`;
  const fallbackSql = embeddedItems ? `SELECT p.id, COALESCE(ei.id, p.document_id) AS document_id,
            p.document_id AS parent_document_id, ei.id AS embedded_item_id,
            COALESCE(ei.title, d.title) AS title, dc.entity_kind, dc.taxonomy_entity_kind, dc.display_facet,
            dc.canonical_subject AS subject,
            COALESCE(dc.canonical_subject, dc.scope_label, dc.source_subject_label) AS entity_label,
     ...
     WHERE p.body LIKE ? ESCAPE '\\'
       ...
       AND (? = '' OR (dc.taxonomy_entity_kind = 'subject' AND dc.canonical_subject = ?))
       AND (? = '' OR COALESCE(ei.stage, d.stage) = ?)
     ORDER BY COALESCE(ei.display_year, d.sort_year) DESC, p.ordinal ASC LIMIT ?` : `SELECT p.id,p.document_id,
            p.document_id AS parent_document_id,NULL AS embedded_item_id,d.title,
            dc.entity_kind,dc.taxonomy_entity_kind,dc.display_facet,dc.canonical_subject AS subject,
            COALESCE(dc.canonical_subject,dc.scope_label,dc.source_subject_label) AS entity_label,
            dc.subject_family,dc.scope_kind,dc.scope_label,d.version_label,
            p.page_number,p.source_locator,p.body,d.source_url,0 AS score
     ...
     WHERE p.body LIKE ? ESCAPE '\\'
       ...
       AND g.citation_allowed=1 AND d.citation_allowed=1
       AND (?='' OR (dc.taxonomy_entity_kind='subject' AND dc.canonical_subject=?))
       AND (?='' OR d.stage=?) ORDER BY d.sort_year DESC,p.ordinal ASC LIMIT ?`;
  const fallback = await env.DB.prepare(fallbackSql)
