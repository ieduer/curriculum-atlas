...
async function createImmutable(request: Request, env: Env): Promise<Response> {
  ...
  if (!existing || !await exactObject(existing, expectedSha256, expectedBytes, releaseId, manifestSha256)) {
...
async function validateStagedRelease(
  ...
  if (!object || !await exactObject(object, manifestSha256, manifestBytes, releaseId, manifestSha256)) return false;
  ...
  for (const entry of declared) {
    ...
    if (!key.startsWith(`${RELEASE_PREFIX}${releaseId}/`) || !SHA256_PATTERN.test(hash)
        || !Number.isSafeInteger(bytes) || bytes <= 0 || expected.has(key)) return false;
    expected.set(key, { sha256: hash, bytes });
  }
  expected.set(manifestKey, { sha256: manifestSha256, bytes: manifestBytes });
  ...
  for (const entry of inventory) {
    ...
    if (!item || entry.sha256 !== item.sha256 || entry.bytes !== item.bytes
        ...
        || entry.metadata_manifest_sha256 !== manifestSha256) return false;
  }
  return true;
}
...
async function exactPointerPredecessor(env: Env, value: Record<string, unknown>): Promise<boolean> {
  ...
  return value.schema_version === 2
    && await exactObject(object, hash, bytes, String(value.release_id || ''), hash);
}
